...
uint8 check_key(uint8 scancode)
{
	uint8 ret = scancode;
...
uint8 get_key() 
{
	...
	switch(scancode)
	{
		...
		case SC_lshft:
		{
			...
			if(check_key(scancode) == 0) return check_key(scancode);
		...
		/* Backspace pressed. */
		case 0xE:return scancode;
		/* Number 1 key released. */
		case 0x82: return 0;
		/* Keypad number 8 pressed. No support for keypad yet. */
		case 0x48: return 0;
		/* Enter pressed. */
		case 0x1C:return scancode;
	...
	if(check_key(scancode) == 0) return 0;
